...
solveMEND <- function(pars,times){
 derivs <- function(t, state, pars){
      with(as.list(c(state, pars)),{
        ...
        fD=0.5 
        gD=0.5
    F1 <- 1/CUE*(Vd+mR)* MB *DOM /( kD + DOM)  ## microbial uptake 
    F2 <- Vp* EP * POM / (kP + POM) # POM decomposition
    F3 <- Vm * EM * MOM / (kM + MOM) #MOAM decomposition
    F4 <- (1/CUE-1)* Vd* MB *DOM /( kD + DOM)
    F5 <- (1/CUE-1)* mR* MB *DOM /( kD + DOM)
    F6 <- Kads * DOM *(1- QOM/ Qmax) ##adsorption
    F7 <- Kdes * QOM/Qmax  ##desorption
    F8 <- (1- pEP- pEM)*mR* MB*MB*0.1 #microbial biomass decay (biomass decay rate=0.2*growth rate)
    F9ep <- pEP * MB *mR  #enzyme production (maintaince respiration=0.1*total respiration)
    F9em <- pEM * MB *mR
    ...
    dPOM <- (1 - gD) * F8- F2
    dMOM <- (1 - fD) * F2 - F3
    dMB <- F1-(F4+F5) - F8 - (F9em + F9ep) # Microbial growth= microbial uptake *CUE
...
print(system.time(Fit <- modFit(p = c(Vd=10, Vp=10, CUE=0.6),
 f = Objective, lower = c(0,0,0.4))))
