...
ctrl$fresh<-ctf$K*1000
ctrl$salt<-cts$K*1000
ctrl<-ctrl[,-1]
...
plot1<-ggplot()+geom_point(obvdf, mapping=aes(time, value, color=variable), size=2)+
 ...
 theme_pubr(border=TRUE)+theme(legend.position="right")+theme(strip.text=element_text(size=18, face="bold"))+xlab("Time(Day)")+ylab("K (mM)")
...
pdf("clay_K.pdf",width=3.8, height=2.5)
plot4
dev.off()
