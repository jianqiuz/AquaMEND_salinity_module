...
plot1<-ggplot()+geom_line(newdf, mapping=aes(time, value, color=variable, linetype=variable),linewidth=1)+ 
  scale_color_manual(values=c("#fdb863","#e08214","#b35806","#7f3b08"))+ scale_linetype_manual(values=c("solid","solid","solid","solid"))+
  scale_y_continuous(labels=percent,limits=c(-0.6,0.6))+
  geom_hline(yintercept=0, linetype="dashed")+
  #scale_x_continuous(breaks=c(0,200,400))+
 theme_pubr(border=TRUE)+theme(legend.position="right")+theme(text=element_text(size=10))+xlab("SC (ds/m)")+ylab("QOM")
plot1

pdf("Ex2_qmax.pdf",width=3.2, height=2)
plot1
dev.off()
#-----testing salinity response-------
df<-data.frame(matrix(NA, nrow=nrow(fs1)))
df$time<-ctrl$SC/1000
df$max1<-(fs1$DOM)/(ctrl$DOM)-1
df$max2<-(fs2$DOM)/(ctrl$DOM)-1
df$max3<-(fs3$DOM)/(ctrl$DOM)-1
df$max4<-(fs4$DOM)/(ctrl$DOM)-1
...
plot1
pdf("Ex2_qmax_IC.pdf",width=3.2, height=2)
plot1
dev.off()
#-----
...
fs1<-read.delim("Rsalt_ref_Ex7_f1.txt",header=TRUE)
fs2<-read.delim("Rsalt_ref_Ex7_f2.txt",header=TRUE)
fs3<-read.delim("Rsalt_ref_Ex7_f3.txt",header=TRUE)
fs4<-read.delim("Rsalt_ref_Ex7_f4.txt",header=TRUE)
obv<-read.csv("Ultisol_control.csv")
